...
class openstack::controller (
  ...
  $ovs_bridge_uplinks      = ["${external_bridge_name}:${bridge_interface}"],
  $ovs_bridge_mappings     = ["default:${external_bridge_name}"],
  ...
  if ($quantum) {
    ...
    class { 'openstack::quantum':
      ...
      ovs_bridge_uplinks      => $bridge_uplinks,
      ovs_bridge_mappings     => $bridge_mappings,
      ovs_network_vlan_ranges => $network_vlan_ranges,
      # Database
